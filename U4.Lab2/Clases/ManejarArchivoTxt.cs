...
namespace Clases
{
    public class ManejarArchivoTxt: ManejadorArchivo
    {
        protected string connectionString {
            get
            {
                return @"Provider=Microsoft.Jet.OLEDB.4.0;
Data Source=C:\Users\bauti\source\mis repos\U4.Lab2\U4.Lab2\bin\Debug;" +
"Extended Properties='text;HDR=Yes;FMT=Delimited'";
        ...
        public override void aplicaCambios()
        {
            using(OleDbConnection Conn = new OleDbConnection(connectionString))
            {
                ...
                cmdInsert.Parameters.Add("@mail", OleDbType.VarChar);
                cmdInsert.Parameters.Add("@telefono", OleDbType.VarChar);

                OleDbCommand cmdUpdate = new OleDbCommand("update agenda.txt set nombre=@nombre, apellido=@apellido, mail=@mail, telefono=@telefono where id=@id",Conn);
                ...
                cmdUpdate.Parameters.Add("@mail", OleDbType.VarChar);
                ...
                if(filasNuevas != null)
                {
                    foreach(DataRow fila in filasNuevas.Rows)
                    {
                        ...
                        cmdInsert.Parameters["@mail"].Value = fila["mail"];
                ...
                if(filasModificadas != null)
                {
                    foreach(DataRow fila in filasModificadas.Rows)
                    {
                        ...
                        cmdUpdate.Parameters["@mail"].Value = fila["mail"];
